...
<div class="charts-row" style="margin-bottom:20px">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <?php if (count($methodBreak) > 0): ?>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:center">
          <canvas id="chartMethods" style="max-height:180px"></canvas>
          <div>
            <?php
            $methodColorMap = ['cash' => '#1D9e75', 'credit_card' => '#378ADD', 'debit_card' => '#7F77DD', 'transfer' => '#EF9F27', 'insurance' => '#D85A30', 'other' => '#888780'];
            foreach ($methodBreak as $mb):
              $color = $methodColorMap[$mb['payment_method']] ?? '#888';
              ?>
              <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
                <div style="display:flex;align-items:center;gap:7px">
                  ...
                  <span
                    style="font-size:0.8rem;color:var(--text-secondary)"><?= $this->model->formatMethod($mb['payment_method']) ?></span>
                </div>
      ...
      <?php else: ?>
        <div class="empty-state" style="padding:32px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
            stroke-width="1.5">
            <circle cx="12" cy="12" r="10" />
          </svg>
          <h3>No data yet</h3>
  ...
  <div class="card-header" style="flex-wrap:wrap;gap:10px">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-left:auto">
      <div class="search-bar" style="max-width:220px">
        ...
        <input type="text" id="txSearch" class="search-input" placeholder="Search patient..."
          value="<?= clean($search) ?>">
      </div>
  ...
  <?php if (count($transactions) > 0): ?>
    <div class="table-wrap">
      <table>
        ...
        <tbody id="txBody">
          <?php
          $methodColors = ['cash' => '#1D9E75', 'credit_card' => '#378ADD', 'debit_card' => '#7F77DD', 'transfer' => '#EF9F27', 'insurance' => '#D85A30', 'other' => '#888780'];
          foreach ($transactions as $t):
            $mColor = $methodColors[$t['payment_method']] ?? '#888';
            ?>
            <tr class="tx-row" data-search="<?= clean(strtolower($t['patient_name'] ?? '')) ?>">
              ...
              <td>
                <span class="method-badge"
                  style="background:<?= $mColor ?>18;color:<?= $mColor ?>;border-color:<?= $mColor ?>30">
                  <?= ['cash' => 'Cash', 'credit_card' => 'Credit Card', 'debit_card' => 'Debit Card', 'transfer' => 'Transfer', 'insurance' => 'Insurance', 'other' => 'Other'][$t['payment_method']] ?? $t['payment_method'] ?>

                </span>
              </td>
              <td style="font-size:0.95rem;font-weight:600;color:var(--text)">
                <?= $c ?>     <?= number_format($t['amount'], 2) ?>
              </td>
              <td style="font-size:0.82rem;color:var(--text-secondary);white-space:nowrap">
                <?= $t['paid_at'] ? formatDate($t['paid_at']) : formatDate($t['created_at']) ?>
              </td>
              <td><?= statusBadge($t['status']) ?></td>
              <td>
                <div style="display:flex;justify-content:flex-end">
                  <?php if ($t['status'] === 'pending'): ?>
                    <form method="POST" action="<?= url('finance/update') ?>" style="display:inline">
                      <?= csrfField() ?>
  ...
  <?php else: ?>
    <div class="card-body">
      <div class="empty-state">
        ...
        <button onclick="openModal('paymentModal')" class="btn btn-primary" style="margin-top:14px">Register
          Payment</button>
      </div>
    </div>
  <?php endif; ?>
    ...
    <form method="POST" action="<?= url('finance/store') ?>">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
          ...
          <?php if (count($pending) > 0): ?>
            <select name="appointment_id" id="apptSelector" class="form-select" required onchange="fillPaymentForm(this)">
              ...
              <?php foreach ($pending as $p): ?>
                <option value="<?= $p['appointment_id'] ?>" data-patient="<?= $p['patient_id'] ?? 0 ?>"
                  data-price="<?= $p['price'] ?>" data-name="<?= clean($p['patient_name']) ?>"
          ...
          <?php else: ?>
            <div
              style="padding:10px;background:var(--gray-50);border-radius:8px;font-size:0.85rem;color:var(--text-muted)">
        ...
        <div id="apptInfo"
          style="display:none;margin-bottom:16px;padding:12px;background:var(--primary-light);border-radius:8px;font-size:0.85rem">
          <div style="font-weight:500;color:var(--primary-dark)" id="apptPatientName"></div>
          <div style="color:var(--text-secondary);margin-top:2px" id="apptProcName"></div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <div style="position:relative">
              <span
                style="position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--text-muted);font-size:0.85rem"><?= $c ?></span>
              <input type="number" name="amount" id="amountInput" class="form-input" step="0.01" min="0.01" required
                placeholder="0.00" style="padding-left:28px">
            </div>
          </div>
          <div class="form-group">
